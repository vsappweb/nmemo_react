...
export default function Rightbar({ user }) {
  ...
  incompleet = Object.values(incompleet).filter(
    (incompleet) => incompleet.hide === false
  );
  ...
  const HomeRightbar = () => {
    return (
      <>
        <Link
          to={`/events/${currentUser.personnelnumber}`}
          style={{ textDecoration: "none" }}
        >
          <h4 className="rightbarTitle">
            {t("rightbar.Here_you_can_view_today's_agenda_events")}
          </h4>
        </Link>
        ...
        {(currentUser.role === 2 || currentUser.role === 0) && (
          <>
            {Object.values(incompleet).length > 0 && (
              <>
                <h4 className="rightbarTitle" style={{ color: "brown" }}>
                  Let op incomplete pallet,
                  <br /> graag aanvullen
                </h4>
                    ...
                    {incomplete.hide === false ? (
                      <>
                        <div className="rightbarInfoItems" key={incomplete._id}>
                          ...
                          <button
                            className="rightbarInfoBtn"
                            onClick={() =>
                              handleIncompletePalletCheck(incomplete._id)
                            }
                          >
                            <VisibilityOff />
                          </button>
          ...
          <>
            ...
            <div className="rightbarFollowings">
              {friends?.map((friend) => {
                return (
                  <Link
                    to={"/profile/" + friend.personnelnumber}
                    style={{ textDecoration: "none" }}
                    key={friend._id}
                  >
                      ...
                      {friend.role === 404 ? (
                        ...
                        <>
                          ...
                          <span className="rightbarFollowingName">
                            {friend.username || friend.personnelnumber}
                          </span>
                        </>
                      )}
                    </div>
