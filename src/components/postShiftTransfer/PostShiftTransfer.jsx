  ...
  useEffect(() => {
    const fetchUser = async () => {
      const res = await axios.get(
        `${API}/users?userId=${shiftTransfer.userId}`
      );
      setUser(res.data);
    };
    fetchUser();
      ...
      <div className="postShiftTransferWrapper">
        ...
        <div className="postShiftTransferTop">
          <div className="postShiftTransferTopLeft">
            ...
            <span className="postShiftTransferPersonnelnumber">
              {user.username || user.personnelnumber}
            </span>
          </div>
          <p className="shiftTransferDateTimeItem">
            Shift: <span>{shiftTransfer?.shift}</span>
          </p>
          <div className="postShiftTransferTopRight">
            {Object.values(allUsers).map((userOperator) => {
              return (
                <Link
                  className="postShiftTransferOperatorLink"
                  to={"/profile/" + userOperator.personnelnumber}
                  style={{ textDecoration: "none" }}
                  key={userOperator._id}
                >
            ...
            {isMenuOpen && (
              <div className="postTopEditDel">
                ...
                shiftTransfer?.shift === shiftNow &&
                shiftTransfer?.date === date.toLocaleDateString("nl-NL") ? (
                  <div className="editDeleteBtns">
                    ...
                    <div
                      className="deleteBtn"
                      onClick={shiftTransferDeleteHandler}
        ...
        <div className="postShiftTransferCenter" style={{ color: "red" }}>
          <ul className="postShiftTransferCenterList">
            {shiftTransfer?.shiftTransferItems.length === 0 ? (
              <li
                className="postShiftTransferCenterItem"
                style={{ color: "green", fontWeight: "bold" }}
              >
                <span>All Okey 😃</span>
              </li>
            ) : null}
            {Object.values(shiftTransfer.shiftTransferItems).map(
              (item, index) => {
                return (
                  <li className="postShiftTransferCenterItem" key={index}>
                    <h3 className="postShiftTransferCenterItemTitle">
                      {item.title}
                    </h3>
                    <p className="postShiftTransferCenterItemValue">
                      {item.value}
                    </p>
                    {item.value === "N.V.T." ? (
                      <span>☹️</span>
          ...
          {shiftTransfer?.message ? (
            <p className="postShiftTransferComment">
              Comment:{" "}
              <span className="postShiftTransferText">
                {shiftTransfer?.message}
              </span>
            </p>
          ) : null}
        ...
        <div
          className="postShiftTransferBottom"
          style={{ display: "flex", flexDirection: "column" }}
        >
          <div
            className="shiftTransferDateTime"
            style={{ textTransform: "capitalize" }}
          >
            <p className="shiftTransferDateTimeItem">
              Week: <span>{shiftTransfer?.weekNumber}</span>
            </p>
            <p className="shiftTransferDateTimeItem">
              Day : <span>{shiftTransfer?.weekday}</span>
            </p>
            <p className="shiftTransferDateTimeItem">
              Date : <span>{shiftTransfer?.date}</span>
            </p>
          </div>
          <span className="postShiftTransferDate">
            {format(shiftTransfer.createdAt)}
          </span>
        </div>
      </div>
    </div>
