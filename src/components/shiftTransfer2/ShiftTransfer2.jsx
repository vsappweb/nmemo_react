...
export default function ShiftTransfer2(shiftTransfer) {
  ...
  const handleRadioChoice = async (shiftTransfer, e) => {
    ...
    if (e.target.value === "NotOk") {
      ...
      document
        .getElementById(`notOk${shiftTransfer._id}`)
        .removeAttribute("style");
      document.getElementById(`N.V.T${shiftTransfer._id}`).style.display =
        "none";
      document.getElementById(`ok${shiftTransfer._id}`).style.display = "none";
      document.getElementById(`editNotOk${shiftTransfer._id}`).style.display =
        "block";
      createItem(values);
      getItem();
      setOpenMessage(true);
    ...
    if (e.target.value === "N.V.T.") {
      ...
      document.getElementById(`ok${shiftTransfer._id}`).style.display = "none";
      document
        .getElementById(`N.V.T${shiftTransfer._id}`)
        .removeAttribute("style");
      document.getElementById(`notOk${shiftTransfer._id}`).style.display =
        "none";
      document.getElementById(`editNVT${shiftTransfer._id}`).style.display =
        "block";
      createItem(values);
      getItem();
      setOpenMessage(true);
    ...
    if (e.target.value === "Ok") {
      ...
      document
        .getElementById(`ok${shiftTransfer._id}`)
        .removeAttribute("style");
      document.getElementById(`N.V.T${shiftTransfer._id}`).style.display =
        "none";
      document.getElementById(`notOk${shiftTransfer._id}`).style.display =
        "none";
      document.getElementById(`editOk${shiftTransfer._id}`).style.display =
        "block";
    ...
    try {
      ...
      for (const key of allKeys) {
        ...
        if (item !== null) {
          ...
          if (
            parsedItem &&
            parsedItem.lineId === user.username &&
            parsedItem.shift === shiftNow &&
            parsedItem.date === date.toLocaleDateString("nl-NL")
          ) {
            res[key] = item;
          }
        }
  ...
  const submitHandler = (e) => {
    e.preventDefault();
    const newShiftTransfer = Object.values(allShiftsTransferValues).map(
      (item) => {
  ...
  const handleEditNotOkItem = (shiftTransfer, e) => {
    ...
    document
      .getElementById(`editNotOk${shiftTransfer._id}`)
      .removeAttribute("style");
  };
  ...
  const handleEditNVTItem = (shiftTransfer, e) => {
    ...
    document.getElementById(`ok${shiftTransfer._id}`).removeAttribute("style");
    document
      .getElementById(`editNVT${shiftTransfer._id}`)
      .removeAttribute("style");
  };
    ...
    <div className="shiftTransfer">
      <div className="shiftTransferDateTime">
        <div className="shiftTransferDateTimeItem">
          Week:{" "}
          <span>
            <WeekNum className="shiftTransferWeekNum" />
          </span>
        </div>
        <p className="shiftTransferDateTimeItem">
          Shift: <span>{shiftNow}</span>
        </p>
        <p className="shiftTransferDateTimeItem">
          Day :{" "}
          <span>{date.toLocaleDateString("nl-NL", { weekday: "long" })}</span>
        </p>
        <p className="shiftTransferDateTimeItem">
          Date : <span>{date.toLocaleDateString("nl-NL")}</span>
        </p>
      </div>
      <form
        className="shiftTransferForm"
        autoComplete="off"
      ...
      >
        {Object.values(allShiftsTransfer).map((shiftTransfer, i) => {
          return (
            <div
              className="shiftTransferItem"
              key={shiftTransfer._id}
              ref={item}
            >
              <div className="shiftTransferContainer">
                <p className="shiftTransferItemNumber" ref={itemNumber}>
                  {i + 1}
                </p>
                <h5 className="shiftTransferItemTitle" ref={title}>
                  {shiftTransfer.title}
                </h5>
                <div className="shiftTransferChoice">
                  <label className="shiftTransferRadioBtnBorder">
                    <input
                      ...
                      // style={{ display: "none" }}
                    />
                    <span className="shiftTransferRadioIconGreen">
                      <TaskAlt id={`ok${shiftTransfer._id}`} />
                    </span>
                    <button
                      className="shiftTransferRadioEditBtn"
                      id={`editOk${shiftTransfer._id}`}
                      onClick={(e) => handleEditOkItem(shiftTransfer, e)}
                    >
                      Edit
                    </button>
                  </label>
                  <label className="shiftTransferRadioBtnBorder">
                    <input
                      ...
                      // style={{ display: "none" }}
                    />
                    <span className="shiftTransferRadioIconRed">
                      <Close id={`notOk${shiftTransfer._id}`} />
                    </span>
                    <button
                      className="shiftTransferRadioEditBtn editNotOkBtn"
                      id={`editNotOk${shiftTransfer._id}`}
                      onClick={(e) => handleEditNotOkItem(shiftTransfer, e)}
                    >
                      Edit
                    </button>
                  </label>
                  <label className="shiftTransferRadioBtnBorder">
                    <input
                      ...
                      // style={{ display: "none" }}
                    />
                    <span className="shiftTransferRadioIconOrange">
                      <DoNotDisturb id={`N.V.T${shiftTransfer._id}`} />
                    </span>
                    <button
                      className="shiftTransferRadioEditBtn editNVTBtn"
                      id={`editNVT${shiftTransfer._id}`}
                      onClick={(e) => handleEditNVTItem(shiftTransfer, e)}
                    >
                      Edit
                    </button>
                  </label>
        ...
        <div className="shiftTransferItem shiftTransferMessage">
          <p className="shiftTransferItemMessage" onClick={handleMessage}>
            {openMessage
              ? "I don't want to leave a message"
              : "I want to leave message"}
          </p>
          {openMessage && (
        ...
        <div className="shiftTransferContainer">
          <label
            className="shiftTransferLabelPersonnelnumber"
            htmlFor="personnelnumber"
          >
            <p
              className="shiftTransferTextPersonnelnumber"
              id="personnelnumberText"
            >
              Operator
            </p>
            <input
              className="shiftTransferInputPersonnelnumber"
              id="personnelnumber"
              ref={operator}
              type="text"
              placeholder="0000"
          ...
          <button className="shiftTransferButton" id="transfer" type="submit">
            Make a shift transfer
          </button>
        </div>
      </form>
    </div>
