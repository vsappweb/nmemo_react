...
import { PermMedia, Try, DoneOutline, EmojiEmotions, Cancel, Today, WorkOutline, Edit, HighlightOff, CleaningServices } from "@mui/icons-material"
            ...
            <div className="memoWrapper">
                ...
                <div className="memoTop">
                    <div className="memoTopMiniSidebar">
                        ...
                        <Today className="memoIcon" htmlColor="tomato" onClick={handleDate} />
                        <WorkOutline className="memoIcon" htmlColor="tomato" onClick={handleOrder} />
                    </div>
                    <div className="memoInputContainer">
                        <textarea className="memoInput" placeholder={"Please leave information " + user.username || user.personnelnumber + "?"} ref={desc} value={text} onChange={(e) => setText(e.target.value)} />
                        {openPreparedTexts ?
                            <div className="memoChoisePreparedText memoChoisePreparedTextScrollbar">
                                <button className="memoChoisePreparedTextCloseBtn" onClick={() => setOpenPreparedTexts(false)}>Close</button>
                                {Object.values(allPreparedTexts).map((preparedText) => {
                                    return (
                                        <div className="memoChoisePreparedTextItem" key={preparedText._id}>
                                ...
                                {user.role === 0 || user.isAdmin ? <form className="preparedTextForm" onSubmit={getPreparedText._id === undefined ? handleClickCreate : handleClickEdit}>
                                    {getPreparedText._id === undefined ? "" : <CleaningServices className="preparedTextFormBtn" onClick={() => setGetPreparedText({})} />}
                                    <label className="preparedTextFormItem" htmlFor="title">
                                        <input className="preparedTextFormInput" type="text" placeholder="New text" id="title" ref={title} defaultValue={"" || getPreparedText.title} required />
                                    </label>
                                    <button className="preparedTextFormBtn" type="submin" >{getPreparedText._id === undefined ? "Create" : "Edit"}</button>
                                </form> : <></>}
                            </div>
                            : <></>}
                    </div>
                ...
                {file && (
                    <div className="memoImgContainer">
                        <img className="memoImg" src={URL.createObjectURL(file)} alt="memoImg" />
                ...
                <form className="memoBottom" onSubmit={editMemo._id === undefined ? submitHandler : editSubmitHandler}>
                    <div className="memoOptions">
                        <label htmlFor="fileMemo" className="memoOption">
                            ...
                            <input style={{ display: "none" }} type="file" id="fileMemo" accept=".png, .jpeg, .jpg" onChange={(e) => setFile(e.target.files[0])} />
                        </label>
                        <label htmlFor="" className="memoOption">
                            <Try className="memoIcon" htmlColor="tomato" onClick={handlePreparedTexts} />
                            <span className="memoOptionText" onClick={handlePreparedTexts}>{openPreparedTexts ? "Hide" : "Show"} texts</span>
                        </label>
