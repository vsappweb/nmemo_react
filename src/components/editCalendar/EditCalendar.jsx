        ...
        {getEvent._id === undefined ? "" : <CleaningServices className="editEventsBtnClean" onClick={eventCleanHandler} />}
          ...
          <form className="editEventsForm" onSubmit={getEvent._id === undefined ? handleClickCreate : handleClickEdit}>
            ...
            <fieldset className="editEventsBorder">
              <legend className="editEventsBorderTitle">{isChecked ? "All day event:" : "Start event:"}</legend>
              <input className="editEventsDate" type="datetime-local" id="startDate" ref={startDate} defaultValue={"" || getEvent.start} required />
              <label className="editEventsFormItem" htmlFor="allDay">
                <p className="editEventsFormText" style={{ marginLeft: "10px" }}>All day:</p>
                <input className='editEventsAllDay' type="checkbox" id="allDay" name="allDay" ref={allDay} defaultValue={isChecked ? "true" : "false"} checked={isChecked}
                  onChange={handleOnChange} />
              </label>
            </fieldset>
            <fieldset className="editEventsBorder" style={isChecked ? { display: "none" } : { display: "flex" }}>
              <legend className="editEventsBorderTitle">End event:</legend>
              <input className="editEventsDate" type="datetime-local" id="endDate" ref={endDate} defaultValue={"" || getEvent.end} />
            </fieldset>
            <label className="editEventsFormItem" htmlFor="desc">
              <p className="editEventsFormText">Description:</p>
              <textarea className="editEventsDesc" placeholder="Write description..." id="desc" ref={desc} defaultValue={"" || getEvent.desc} ></textarea>
            </label>
            <button className="editEventsBtn" type="submin" >{getEvent._id === undefined ? "Create" : "Edit"}</button>
          </form>
                ...
                return (
                  <div className="editEventsRightItem" key={event._id}>
                    <div className="editBtn">
                      <Edit className="eventEdit" onClick={() => eventGetHandler(event)} />
                    </div>
                    ...
                    <div className="deleteBtn">
                      <HighlightOff className="eventDelete" onClick={() => eventDeleteHandler(event)} />
                    </div>
                  </div>
                )
              })}
