...
export default function Stempel() {
  ...
  //add unit https://sadam-bapunawar.medium.com/add-and-remove-form-fields-dynamically-using-react-and-react-hooks-3b033c3c0bf5
  // add unit https://sadam-bapunawar.medium.com/add-and-remove-form-fields-dynamically-using-react-and-react-hooks-3b033c3c0bf5
  const [formValues, setFormValues] = useState([{ productName: "", stempel: "" }]);
                ...
                <>
                  <div className="stempelContainerItems" key={i}>
                    <fieldset className="stempelFormType">
                      <legend className="editProfileBorderTitle">Names of product and stempels:</legend>
                      {stempel.productNameAndStempel && Object.values(stempel.productNameAndStempel).map((nameAndStempel, index) => {
                        return (
                          <>
                            <div className="stempelContainerItem" key={index}>
                              <p className="stempelFormText">Product Name {index + 1}:</p>{showStempel && <p>{nameAndStempel.productName || ''}</p>}
                              {showEditStempel && nameAndStempel && <input className="stempelFormInput" name='productName' type="text" id='productName' placeholder={`Product name ${index + 1}`} defaultValue={nameAndStempel.productName || ''} onChange={e => handleChange(index, e)} />}
                            </div>
                            <div className="stempelContainerItem">
                              <p className="stempelFormText">Stempel {index + 1}:</p>{showStempel && <p style={{ color: 'blue' }}>{nameAndStempel.stempel || ''}</p>}
                              {showEditStempel && nameAndStempel && <input className="stempelFormInput" name='stempel' type="text" id='stempel' placeholder={`Stempel ${index + 1}`} defaultValue={nameAndStempel.stempel || ''} onChange={e => handleChange(index, e)} />}
                            </div>
                          </>
                        )
                      })}
                    ...
                    <fieldset className="stempelFormType">
                      <legend className="editProfileBorderTitle">Units:</legend>
                      {stempel.units && Object.values(stempel.units).map((unit, index) => {
                        return (
                          <>
                            <div className="stempelContainerItem" key={index}>
                              <p className="stempelFormText">Unit {index + 1}:</p>{showStempel && <p>{unit.unit || ''}</p>}
                              {showEditStempel && unit && <input type="text" name={`Unit`} placeholder={`Unit ${index + 1}`} defaultValue={unit.unit || ''} onChange={e => handleChangeUnit(index, {e: unit.unit })} />}
                            </div>
                          </>
                        )
                      })}
                  ...
                  <div className="stempelFormBtn">
                    ...
                    {showEditStempel && stempel && <button className="buttonBtn" type="submit" onClick={() => handleEdit(stempel._id)}  >"Edit"</button>}
                    {delBtn && stempel && <button className="buttonBtn" type="submit" onClick={() => handleDelete(stempel._id)} >"Delete"</button>}
                    <div className="deleteBtn" onClick={() => handleChangeBtn()}>
                      <HighlightOff />
                    </div>
                  </div>
                </>
              ...
              <form className="stempelForm" onSubmit={handleSubmit} >
                ...
                {formValues.map((element, index) => (
                  <div className="form-inline" key={index}>
                    <fieldset className="stempelFormType">
                      <legend className="stempelFormTypeTitle">Names of product and stempels:
                        {
                          index ?
                      ...
                      <label className="stempelFormLabel" htmlFor="productName">
                        ...
                        <input className="stempelFormInput" name='productName' type="text" id='productName' ref={productName} placeholder={`Product name ${index + 1}`} defaultValue={element.productName || ''} onChange={e => handleChange(index, e)} required />
                      </label>
                      <label className="stempelFormLabel" htmlFor="stempel">
                        <p className="stempelFormText">Stempel:</p>
                        <input className="stempelFormInput" name='stempel' type="text" id='stempel' ref={stempel} placeholder={`Stempel ${index + 1}`} defaultValue={element.stempel || ''} onChange={e => handleChange(index, e)} required />
                      </label>
                    </fieldset>
                  </div>
                ))}
                  ...
                  {unitValues.map((element, index) => (
                    <div className="form-inline" key={index}>
                      <label className="stempelFormLabel" htmlFor="unit">
                        ...
                        {
                          index ?
                            <div className="deleteBtn" onClick={() => removeUnitFields(index)}>
                              <Remove />
                            </div>
                            :
                            <div className="editBtn" onClick={() => addUnitFields()}>
                              <Add />
                            </div>
                        }
                        <input type="text" name='unit' placeholder={`Unit ${index + 1}`} defaultValue={element.unit || ""} onChange={e => handleChangeUnit(index, e)} />
                      </label>
                    </div>
                  ))}
                </fieldset>
