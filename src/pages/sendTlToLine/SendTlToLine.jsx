              ...
              return (
                <li className="feedTlToLineInformation" key={toLines._id}>
                  ...
                  <div className="feedTlToLineInformationContent">
                    <p className="feedTlToLineInformationHeader">
                      {toLines.line}{" "}
                      <sup
                        style={{
                          display: toLines?.reqRes ? "inline" : "none",
                          fontStyle: "italic",
                          fontSize: "10px",
                          color: "red",
                        }}
                    ...
                    <p className="feedTlToLineInformationDesc">
                      {toLines?.desc}
                    </p>
                    {toLines?.img && (
                      <img
                        className="postMemoImg"
                        src={PF + toLines?.img}
                        alt=""
                      />
                    )}
                    ...
                    {toLines.line === "forAll" && toLines.reqRes && (
                      <div className="feedTlToLineInformationAnswerContainer">
                        {toLines?.agrees && toLines.agrees.length > "0" && (
                          <p
                            className="feedTlToLineInformationAnswer"
                            style={{ color: "green" }}
                          >
                            Okey: <span>{toLines?.agrees.length}</span>
                          </p>
                        )}
                        {toLines?.disagrees &&
                          toLines.disagrees.length > "0" && (
                            <p
                              ...
                              style={{ color: "red" }}
                            >
                              Not Okey: <span>{toLines?.disagrees.length}</span>
                            </p>
                          )}
                      </div>
                    )}
                    <div className="sendTlToLineInformationBox">
                      ...
                      <p className="feedTlToLineInformationDesc">
                        {toLines?.timer < time && eventDeleteHandler(toLines)}
                      </p>
                    </div>
                  </div>
                  <div className="editEventsRightItemDeleteBtn">
                    <HighlightOff
                      ...
                      onClick={() => eventDeleteHandler(toLines)}
                    />
                  </div>
                </li>
              );
